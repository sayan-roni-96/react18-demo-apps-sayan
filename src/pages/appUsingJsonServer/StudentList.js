...
const StudentList = () => {
    ...
    const getAllstudentlist = ()=>{
        axios.get(`${process.env.REACT_APP_NEW_JSON_URL}/student`)
        .then((resp) => {
  ...
  return (
    <div className="container mt-4">
      ...
      <thead>
       ...
       {allStudents && allStudents.map((sData,index)=> {
            return (
                <tbody key={sData.id}>
                <tr>
                  <td>{index+1}</td>
                  ...
                  <td><Button variant="secondary" onClick={() => viewSClick(sData)}>View</Button>{' '}
                      <Button variant="success">Edit</Button>{' '}
                      <Button variant="warning" onClick={() => delClick(sData)}>Delete</Button>{' '}</td>
                </tr>
